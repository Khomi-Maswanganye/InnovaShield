...
async function migrateWatchlistUserId() {
    try {
        const columns = await dbQ("SHOW COLUMNS FROM watchlist LIKE 'user_id'");
        if (!columns.length) return;
        const colType = String(columns[0].Type || '').toLowerCase();
        if (colType.includes('int')) return;

        console.log('🔧 Migrating watchlist.user_id from username strings to numeric user IDs...');
        await dbQ('ALTER TABLE watchlist ADD COLUMN user_id_ref INT NULL');
        await dbQ('UPDATE watchlist w JOIN users u ON w.user_id=u.username SET w.user_id_ref=u.user_id');
        await dbQ('DELETE w FROM watchlist w LEFT JOIN users u ON w.user_id=u.username WHERE u.user_id IS NULL');
        await dbQ('ALTER TABLE watchlist DROP COLUMN user_id');
        await dbQ('ALTER TABLE watchlist CHANGE COLUMN user_id_ref user_id INT NOT NULL');
        await dbQ('ALTER TABLE watchlist ADD CONSTRAINT fk_watchlist_user_id FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE');
        console.log('✅ watchlist.user_id migration completed.');
    } catch(e) {
        console.warn('⚠️ watchlist user_id migration skipped or failed:', e.message);
    }
}
...
app.get('/account', requireLogin, asyncHandler(async (req, res) => {
    try {
...
app.post('/account', requireLogin, asyncHandler(async (req, res) => {
...
app.post('/account/delete', requireLogin, asyncHandler(async (req, res) => {
...
app.get('/admin', requireLogin, requireRole('Admin'), asyncHandler(async (req, res) => {
    try {
...
app.get('/admin/report/export/csv', requireLogin, requireRole('Admin'), asyncHandler(async (req, res) => {
    try {
...
app.get('/admin/report/export/pdf', requireLogin, requireRole('Admin'), asyncHandler(async (req, res) => {
    try {
        const dateFrom = req.query.date_from || new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0];
        const dateTo = req.query.date_to || new Date().toISOString().split('T')[0];
        const reportType = req.query.report_type || 'all';
...
app.get('/admin/users', requireLogin, requireRole('Admin'), asyncHandler(async (req, res) => {
    try {
...
app.post('/admin/users/delete/:id', requireLogin, requireRole('Admin'), asyncHandler(async (req, res) => {
    try {
...
app.post('/admin/users/role/:id', requireLogin, requireRole('Admin'), asyncHandler(async (req, res) => {
    try {
...
app.get('/', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (req.session.user) {
            const watchlistRows = await dbQ('SELECT patent_id, trademark_id FROM watchlist WHERE user_id=?', [req.session.user.user_id]);
            watchlistRows.forEach(row => {
                if (row.patent_id) watchlistLookup.add(`patent_${row.patent_id}`);
                if (row.trademark_id) watchlistLookup.add(`trademark_${row.trademark_id}`);
            });
        } else if (req.session.guestWatchlist) {
            req.session.guestWatchlist.forEach(item => {
                if (item.patent_id) watchlistLookup.add(`patent_${item.patent_id}`);
                if (item.trademark_id) watchlistLookup.add(`trademark_${item.trademark_id}`);
            });
        }

        const results = [...patents, ...tms]
            .map(p => {
                const selectionValue = `${p.record_type.toLowerCase()}_${p.record_type === 'Trademark' ? p.trademark_id : p.patent_id}`;
                const legalDate = p.record_type === 'Trademark' ? p.registration_date : p.filing_date;
                const legalLabel = p.record_type === 'Trademark' ? 'Registration Date' : 'Filed';
                const summarySource = p.description || '';
                const fallbackSummary = p.record_type === 'Trademark'
                    ? `Trademark "${p.display_title}" owned by ${p.owner || 'Unknown'} in ${p.industry || 'Industry not set'}. Status: ${p.status || 'Unknown'}.`
                    : `Patent "${p.display_title}" owned by ${p.owner || 'Unknown'} in ${p.industry || 'Industry not set'}. Status: ${p.status || 'Unknown'}.`;
                const summaryText = summarySource.trim() || fallbackSummary;
                const expiresText = p.expiry_date ? new Date(p.expiry_date).toLocaleDateString() : 'TBD';
                let durationText = 'Not active';
                if (p.status === 'Active' && p.expiry_date) {
                    const now = new Date();
                    const expiry = new Date(p.expiry_date);
                    const diffDays = Math.round((expiry - now) / (1000 * 60 * 60 * 24));
                    durationText = diffDays >= 0 ? `${Math.max(diffDays, 0)} day(s)` : 'Expired';
                }
                const summaryHtml = String(summaryText)
                    ...
                    .replace(/\n/g, '<br/>');
                const detailHtml = `
                    <div class="summary-detail-row"><strong>Owner</strong><span>${(p.owner || 'Unknown')}</span></div>
                    <div class="summary-detail-row"><strong>Name</strong><span>${(p.display_title || 'Untitled')}</span></div>
                    <div class="summary-detail-row"><strong>Status</strong><span>${(p.status || 'Unknown')}</span></div>
                    <div class="summary-detail-row"><strong>${legalLabel}</strong><span>${legalDate ? new Date(legalDate).toLocaleDateString() : 'Unknown'}</span></div>
                    <div class="summary-detail-row"><strong>Expires</strong><span>${expiresText}</span></div>
                    <div class="summary-detail-row"><strong>Active duration</strong><span>${durationText}</span></div>
                    <div class="summary-detail-row"><strong>Industry</strong><span>${(p.industry || 'Unknown')}</span></div>
                    <div class="summary-detail-copy"><strong>Summary</strong><p>${summaryHtml}</p></div>
                `;

                return {
                    ...p,
                    selectionValue,
                    in_watchlist: watchlistLookup.has(selectionValue),
                    summary_html: summaryHtml,
                    detail_html: detailHtml
                };
            })
            .sort((a, b) => {
                const aTitle = (a.display_title || '').toLowerCase();
                const bTitle = (b.display_title || '').toLowerCase();
...
// ─── PATENTS ──────────────────────────────────────────────────────────────────
app.get('/patents', requireLogin, requireRole('Admin', 'Analyst'), asyncHandler(async (req, res) => {
...
app.get('/patents/export/csv', requireLogin, requireRole('Admin', 'Analyst'), asyncHandler(async (req, res) => {
    try {
...
app.post('/patents/delete/:id', requireLogin, requireRole('Admin','Analyst'), asyncHandler(async (req, res) => {
...
// ─── TRADEMARKS ───────────────────────────────────────────────────────────────
app.get('/trademarks', requireLogin, requireRole('Admin', 'Analyst'), asyncHandler(async (req, res) => {
    const search = req.query.q || '';
    const source = req.query.source || '';
    const statusFilter = req.query.status || '';
    const now = new Date();
...
app.get('/trademarks/export/csv', requireLogin, requireRole('Admin', 'Analyst'), asyncHandler(async (req, res) => {
    try {
...
app.post('/trademarks/delete/:id', requireLogin, requireRole('Admin','Analyst'), asyncHandler(async (req, res) => {
...
// ─── WATCHLIST ────────────────────────────────────────────────────────────────
app.get('/watchlist', asyncHandler(async (req, res) => {
    try {
        ...
        if (req.session.user) {
            ...
            const wParams = [];
            if (!isAdmin) { wSql += ' WHERE w.user_id=?'; wParams.push(req.session.user.user_id); }
...
app.post('/watchlist/add', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        await dbQ('INSERT INTO watchlist (patent_id, trademark_id, user_id, notes) VALUES (?,?,?,?)',
            [type === 'patent' ? patent_id : null, type === 'trademark' ? trademark_id : null, req.session.user.user_id, notes || null]);
        res.redirect('/watchlist?success=added');
    } catch(e) { req.flash('error', 'Could not add to watchlist.'); res.redirect('/watchlist'); }
}));

app.get('/watchlist/edit/:id', requireLogin, asyncHandler(async (req, res) => {
    try {
        const rows = await dbQ('SELECT * FROM watchlist WHERE watchlist_id=?', [req.params.id]);
        if (!rows.length) return res.redirect('/watchlist');
        const item = rows[0];
        if (req.session.user.role !== 'Admin' && item.user_id !== req.session.user.user_id) {
...
app.post('/watchlist/update', requireLogin, asyncHandler(async (req, res) => {
...
app.post('/watchlist/add-selected', asyncHandler(async (req, res) => {
    ...
    const sendResponse = (status, success, message, redirectUrl) => {
        if (isAjax) {
            return res.status(status).json({ success, message, redirectUrl });
        }
        if (redirectUrl) return res.redirect(redirectUrl);
        return success ? res.redirect(backUrl) : res.redirect(backUrl);
    };

    if (!selected_item) {
        return sendResponse(400, false, 'Please select an item to add to watchlist.');
    }

    const [type, id] = selected_item.split('_');
    if (!type || !id || (type !== 'patent' && type !== 'trademark')) {
        return sendResponse(400, false, 'Invalid selection.');
    }

    try {
        if (!req.session.user) {
            if (!req.session.guestWatchlist) req.session.guestWatchlist = [];
            if (req.session.guestWatchlist.some(item => item.type === type && ((type === 'patent' && item.patent_id === id) || (type === 'trademark' && item.trademark_id === id)))) {
            ...
            req.session.guestWatchlist.push({
                type,
                patent_id: type === 'patent' ? id : null,
                trademark_id: type === 'trademark' ? id : null,
                notes: null,
                user_id: 'Anonymous',
                created_at: new Date().toISOString()
            });
            return sendResponse(200, true, 'Added to watchlist!');
        }

        const existing = await dbQ('SELECT watchlist_id FROM watchlist WHERE user_id=? AND ' +
            (type === 'patent' ? 'patent_id=?' : 'trademark_id=?'), [req.session.user.user_id, id]);
        if (existing.length > 0) {
            return sendResponse(409, false, 'This item is already in your watchlist.');
        }

        await dbQ('INSERT INTO watchlist (patent_id, trademark_id, user_id, notes) VALUES (?,?,?,?)',
            [type === 'patent' ? id : null, type === 'trademark' ? id : null, req.session.user.user_id, null]);
        return sendResponse(200, true, 'Added to watchlist!');
    } catch(e) {
        console.error(e);
        return sendResponse(500, false, 'Could not add to watchlist.');
    }
}));

app.get('/watchlist/export/csv', requireLogin, asyncHandler(async (req, res) => {
    try {
        const isAdmin = req.session.user.role === 'Admin';
        ...
        const params = [];
        if (!isAdmin) { sql += ' WHERE w.user_id=?'; params.push(req.session.user.user_id); }
...
// ─── EXPIRING ─────────────────────────────────────────────────────────────────
app.get('/expiring', requireLogin, asyncHandler(async (req, res) => {
    const now = new Date();
...
app.get('/expiring/export/csv', requireLogin, asyncHandler(async (req, res) => {
    try {
        const now = new Date();
        const defaultTo = new Date(now.getTime() + 180*24*60*60*1000).toISOString().split('T')[0];
...
// ─── ALERTS ───────────────────────────────────────────────────────────────────
app.get('/alerts', requireLogin, asyncHandler(async (req, res) => {
    try {
...
// ─── TRENDS ───────────────────────────────────────────────────────────────────
app.get('/trends', requireLogin, asyncHandler(async (req, res) => {
    try {
...
app.get('/trends/export/csv', requireLogin, asyncHandler(async (req, res) => {
    try {
...
app.post('/api/sync', requireLogin, requireRole('Admin','Analyst'), asyncHandler(async (req, res) => {
    const counts = await runAutoSync(req.body.topic || 'artificial intelligence');
    res.json({ success: true, synced: counts, time: new Date() });
}));

app.get('/api/sync-status', requireLogin, asyncHandler(async (req, res) => {
    try {
...
app.get('/api/patents', requireLogin, requireRole('Admin'), asyncHandler(async (req, res) => {
    try { const r = await dbQ('SELECT * FROM patents'); res.json({ success: true, count: r.length, data: r }); }
    catch(e) { res.status(500).json({ error: 'DB error' }); }
}));

app.get('/api/trademarks', requireLogin, requireRole('Admin'), asyncHandler(async (req, res) => {
...
const SERVER_HOST = process.env.HOST || process.env.SERVER_HOST || '0.0.0.0';
const SERVER_PORT = Number(process.env.PORT || process.env.SERVER_PORT || 3000);
app.listen(SERVER_PORT, SERVER_HOST, () => {
    const visibleHost = SERVER_HOST === '0.0.0.0' ? '0.0.0.0 (all interfaces)' : SERVER_HOST;
    console.log(`Server running on http://${visibleHost}:${SERVER_PORT}`);
    console.log(`Remote clients can connect to http://<server-ip>:${SERVER_PORT}`);
});